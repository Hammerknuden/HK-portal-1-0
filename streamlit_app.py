...
if year == '2024':
  ...
  #URL = https://docs.google.com/spreadsheets/d/1-9mALBucV-67ytDpfsWD9vUwXA_-tHOj/edit?usp=sharing&ouid=113417285199087461988&rtpof=true&sd=true
...
def highlight_cells(x):
    ...
    return df.map(lambda v: 'background-color: #66FF66' if v == 'F' else '') #66FF66

styled_data = new_data.style.apply(highlight_cells, axis=None)
...
if year == '2025':
    high_season_start = datetime.strptime("24-06-25", _format := "%d-%m-%y").date()
    high_season_end = datetime.strptime("24-08-24", _format := "%d-%m-%y").date()
if year == '2025':
    high_season_start = datetime.strptime("23-06-25", _format := "%d-%m-%y").date()
    high_season_end = datetime.strptime("23-08-24", _format := "%d-%m-%y").date()

# dates 2025 24juni2025 - 24aug25
days = checkout_date - checkin_date
...
if web == "web":
    ...
    formatted_pris_add_t = f"{pris_add_t:.2f}kr"
    st.markdown(f"**Tiilæg** {formatted_pris_add_t}")
